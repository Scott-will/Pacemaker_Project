...
class Parameter_Window:
    def __init__(self, master, mode, user):
        ...
        if self.mode == 1:
            ...
            self.background_image = tk.PhotoImage(file="backgroundpacingAOO.png")
            self.label = Label(self.frame_root, image=self.background_image)
            self.label.image = self.background_image
            self.label.pack()
            ...
            self.label_title1.place(x=50, y=150)
            ...
            # atrial amp atrial pulse width, upper limit(120 for ex), lower limit(60)
            ...
            self.write_parameters()
            # Lower limit, upper limit, atrial amp, atrial pulse width, Atrial sensitivity, ARP, PVARP, Hysteresis, rate smoothing
            ...
            self.entry_Rate_Smooth.place(x=650, y=422)
            self.write_parameters()
            # Lower limit, upper limit, vent amp, vent pulse width,vent sensitivity, VRP, Hysteresis, rate smoothing

    def common(self,upperlimx,upperlimy,lowerlimx,lowerlimy,upperlimx2,upperlimy2,lowerlimx2,lowerlimy2,backx,backy,okx,oky,applyx,applyy):
